mergeInto(LibraryManager.library, {
  LudoSocketIoBridge_Connect: function (objectNamePtr, serverBaseUrlPtr, allowReconnect) {
    ...
    if (!bridge) {
      bridge = window.PremiumLudoSocketBridge = {
        objectName: "",
        serverBaseUrl: "",
        allowReconnect: true,
        socket: null,
        ...
        notify: function (methodName, payload) {
          ...
          window.setTimeout(function () {
            try {
              ...
              if (typeof window.PremiumLudoShowError === "function") {
                window.PremiumLudoShowError(
                  error && error.message
                    ? error.message
                    : "Browser bridge failed while updating multiplayer."
                );
              }
            }
          }, 0);
